  ...
  return (
    <div className="pt-6 border-t border-gray-400/50">
        ...
        <div className="flex space-x-2 overflow-x-auto pb-3 mb-3 scrollbar-hide">
            {['Top', 'Bottom', 'Footwear', 'Watch', 'Glasses'].map((category) => (
                <label key={category} className={`flex-shrink-0 flex items-center px-3 py-2 border rounded-full text-sm font-medium transition-colors ${isLoading ? 'cursor-not-allowed bg-gray-100 text-gray-400 border-gray-200' : 'hover:bg-gray-50 hover:border-gray-400 text-gray-700 border-gray-300 cursor-pointer'}`}>
                    <UploadCloudIcon className="w-4 h-4 mr-1.5"/>
                    <span>{category}</span>
                    <input type="file" className="hidden" accept="image/png, image/jpeg, image/webp, image/avif, image/heic, image/heif" onChange={handleFileChange(category)} disabled={isLoading}/>
                </label>
            ))}
        </div>
        ...
        <div className="grid grid-cols-3 gap-3">
            ...
            <label htmlFor="custom-garment-upload" className={`relative aspect-square border-2 border-dashed rounded-lg flex flex-col items-center justify-center text-gray-500 transition-colors ${isLoading ? 'cursor-not-allowed bg-gray-100' : 'hover:border-gray-400 hover:text-gray-600 cursor-pointer'}`}>
                ...
                <span className="text-xs text-center">Upload Any</span>
                <input id="custom-garment-upload" type="file" className="hidden" accept="image/png, image/jpeg, image/webp, image/avif, image/heic, image/heif" onChange={handleFileChange('Custom')} disabled={isLoading}/>
            </label>
        </div>
        {wardrobe.length === 0 && (
